...
def query_endpoint_documents(endpoint_url: str = r"https://www.federalregister.gov/api/v1/documents.json?"):
    """Queries the GET documents.{format} endpoint of the Federal Register API.

    Args:
        endpoint_url (_type_, optional): Endpoint for retrieving documents. Defaults to r"https://www.federalregister.gov/api/v1/documents.json?".
    ...
    fieldsList = ["publication_date", "president", "agencies", "agency_names", 
                  ...
                  "type", "action", "dates", 
                  "json_url", "correction_of"
                  ]
    years = [(2016, 2017), (2020, 2021)]  # midnight period spans from Nov. to Jan.
    ...
    for year in years:
        ...
        start_date = f"{year[0]}-11-01"
        end_date = f"{year[1]}-01-31"

        # update parameters for year
        dict_params.update({"conditions[publication_date][gte]": start_date, 
